...
/**
 * وكيل ذكي (شات بوت) للمدرسة — يسمح بسؤال أسئلة عن بيانات طلاب المدرسة ومؤشرات الابتكار الخاصة بهم فقط.
 */
class SchoolChatController extends Controller
{
    ...
    {
        return Inertia::render('School/Innovation/SchoolChat');
    }
        ...
        try {
            ...
            $answer = $this->client->chat([
                GeminiClient::systemMessage(
                    ...
                    . "تجيب على أسئلة مسؤول المدرسة حول بيانات طلاب مدرسته ومؤشرات الابتكار والإحصائيات الخاصة بهم فقط. "
            ...
            Log::error("School Chat AI error: {$e->getMessage()}");
    ...
    private function buildContext(): string
    {
        ...
        $studentIds = User::where('role', 'student')->where('school_id', $schoolId)->pluck('id');

        $totalStudents = $studentIds->count();
        $assessed = InnovationIndex::whereIn('user_id', $studentIds)
            ->selectRaw('COUNT(DISTINCT user_id) as total')
            ->value('total');

        $latestIndexIds = function ($q) use ($studentIds) {
            $q->selectRaw('MAX(id)')->from('innovation_indexes')->whereIn('user_id', $studentIds)->groupBy('user_id');
        };

        $classifications = InnovationIndex::query()
            ->selectRaw('classification, COUNT(*) as cnt')
            ->whereIn('id', $latestIndexIds)
        ...
        return <<<CTX
إجمالي طلاب المدرسة: {$totalStudents}
...
أعلى 5 طلاب في المدرسة:
{$topStudents}

أضعف 5 طلاب في المدرسة:
{$weakStudents}
CTX;
    }
}
