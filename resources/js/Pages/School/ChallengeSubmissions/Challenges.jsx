...
export default function SchoolChallengeSubmissionsChallenges({ auth, challenges }) {
    ...
    const getStatusBadge = (challenge) => {
        ...
        if (challenge.status === 'active' && startDate <= now && deadline >= now) {
            return { bg: 'bg-green-100', text: 'text-green-800', label: t('challenges.active') };
        }
        if (challenge.status === 'active' && startDate > now) {
            return { bg: 'bg-blue-100', text: 'text-blue-800', label: t('challenges.upcoming') };
        }
        return { bg: 'bg-gray-100', text: 'text-gray-800', label: t('challenges.finished') };
    };

    return (
        <DashboardLayout auth={auth}>
            <Head title={t('schoolChallengeSubmissionsPage.pageTitle', { appName: t('common.appName') })} />

            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="mb-8">
                    <h1 className="text-3xl font-bold text-gray-900 mb-2">{t('schoolChallengeSubmissionsPage.title')}</h1>
                    <p className="text-gray-600">{t('schoolChallengeSubmissionsPage.subtitle')}</p>
                </div>
                ...
                {challenges.data && challenges.data.length > 0 ? (
                                ...
                                >
                                    ...
                                    <div className="flex items-center justify-between text-sm text-gray-600 mb-4">
                                        <div className="flex items-center gap-1">
                                            <FaUsers />
                                            <span>{t('schoolChallengeSubmissionsPage.submissionsCount', { count: challenge.submissions_count || 0 })}</span>
                                        </div>
                                        <div className="flex items-center gap-1">
                                            <FaCalendar />
                                    ...
                                    <div className="flex items-center gap-2 text-blue-600 hover:text-blue-700">
                                        <FaEye />
                                        <span className="text-sm font-medium">{t('schoolChallengeSubmissionsPage.viewSubmissions')}</span>
                                    </div>
                                </Link>
                            );
                        })}
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
                        ...
                        <p className="text-gray-500 text-lg">{t('schoolChallengeSubmissionsPage.noChallenges')}</p>
                    </div>
                )}
