...
import { FaCalendar, FaPlus, FaEdit, FaTrash, FaClock, FaCheck, FaTimes, FaArrowRight } from 'react-icons/fa';
import { useState } from 'react';

export default function Availabilities({ teacher, availabilities, subjects = [], auth }) {
    ...
    const handleSubmit = (e) => {
        ...
        if (editingAvailability) {
            put(`/admin/teachers/${teacher.id}/availabilities/${editingAvailability.id}`, {
                onSuccess: () => {
            ...
            post(`/admin/teachers/${teacher.id}/availabilities`, {
                onSuccess: () => {
                    setShowAddModal(false);
                    reset();
                }
    ...
    const handleDeleteConfirm = () => {
        if (availabilityToDelete) {
            destroy(`/admin/teachers/${teacher.id}/availabilities/${availabilityToDelete.id}`, {
                onSuccess: () => {
    ...
    return (
        <DashboardLayout header={`إدارة مواعيد ${teacher.name_ar || teacher.name}`}>
            <Head title={`إدارة مواعيد ${teacher.name_ar || teacher.name}`} />
            ...
            <div className="mb-6 flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">
                        إدارة مواعيد {teacher.name_ar || teacher.name}
                    </h1>
                    <p className="text-gray-600 mt-1">{teacher.user?.email}</p>
                </div>
                <button
                    onClick={() => {
                        setEditingAvailability(null);
                                    ...
                                    {groupedAvailabilities[selectedDate].map(availability => (
                                        ...
                                        >
                                            ...
                                            {availability.subject && (
                                                <p className="text-xs text-gray-600">المادة: {availability.subject.name_ar || availability.subject.name_en}</p>
                                            )}
                                        </div>
                                    ))}
                                </div>
