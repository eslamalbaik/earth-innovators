...
export default function AdminCertificatesEdit({ certificate, users }) {
    const { data, setData, post, processing, errors } = useForm({
        user_id: certificate.user_id || '',
        type: certificate.type || 'student',
        title: certificate.title || '',
        title_ar: certificate.title_ar || '',
        description: certificate.description || '',
        description_ar: certificate.description_ar || '',
        issue_date: certificate.issue_date ? new Date(certificate.issue_date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
    ...
    return (
        <DashboardLayout header="تعديل الشهادة">
            <Head title="تعديل الشهادة" />
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
                <div className="mb-6">
                    ...
                    <p className="text-sm text-gray-600 mt-1">رقم الشهادة: {certificate.certificate_number}</p>
                </div>
                    ...
                    {certificate.file_path && (
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                            <p className="text-sm text-blue-800">
                                <strong>ملف PDF موجود:</strong> {certificate.file_path}
                            </p>
                        </div>
                    )}
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center">
                            <input
                                ...
                                onChange={(e) => setData('regenerate_pdf', e.target.checked)}
                                className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                            />
                            <label htmlFor="regenerate_pdf" className="mr-2 block text-sm text-gray-900">
                                إعادة إنشاء ملف PDF
                            </label>
                        </div>
                    </div>
