...
export default function AdminPackagesEdit({ package: pkg }) {
    ...
    const [baseMonthlyPrice, setBaseMonthlyPrice] = useState(() => {
        const months = Math.max(1, Number(pkg.duration_months) || 1);
        const students = Math.max(1, Number(pkg.students_count) || 1);
        const totalPrice = Number(pkg.price) || 0;
        return Number((totalPrice / (months * students)).toFixed(2));
    });
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        students_count: pkg.students_count || 1,
        grade: pkg.grade || '',
        class_section: pkg.class_section || '',
    ...
    return (
        <DashboardLayout header={t('adminPackagesEditPage.title')}>
            <Head title={t('adminPackagesEditPage.pageTitle', { appName: t('common.appName') })} />
            ...
            <div className="mb-6">
                ...
                >
                    ...
                    {t('adminPackagesEditPage.actions.backToPackages')}
                </Link>
            </div>

            <div className="bg-white rounded-xl shadow-lg p-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-6">{t('adminPackagesEditPage.sections.packageInfo')}</h2>
                ...
                <form onSubmit={submit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                {t('adminPackagesEditPage.fields.projectsLimit')}
                            </label>
                            <input
                                type="number"
                                min="0"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">
                                {t('adminPackagesEditPage.fields.challengesLimit')}
                            </label>
                            <input
                                type="number"
                                min="0"
