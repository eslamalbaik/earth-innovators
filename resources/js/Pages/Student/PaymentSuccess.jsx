            ...
            {showToast && (
                <div className="fixed bottom-4 left-4 right-4 md:left-auto md:right-4 md:w-96 z-50">
                    <div className="bg-green-500 text-white rounded-lg shadow-lg p-4 flex items-center justify-between">
                        <p>{t('studentPaymentPage.successToast')}</p>
                        <button
            ...
            <div className="max-w-2xl mx-auto">
                <div className="bg-white rounded-xl shadow-lg p-8 text-center">
                    <div className="mb-6">
                        ...
                        <h1 className="text-3xl font-bold text-gray-900 mb-2">{t('studentPaymentPage.successTitle')}</h1>
                        <p className="text-gray-600">{t('studentPaymentPage.successDescription')}</p>
                    </div>

                    <div className="bg-gray-50 rounded-lg p-6 mb-6 ">
                        <h2 className="text-lg font-bold text-gray-900 mb-4">{t('studentPaymentPage.detailsTitle')}</h2>
                        <div className="space-y-3">
                            <div className="flex justify-between">
                                <span className="text-gray-600">{t('studentPaymentPage.transactionIdLabel')}</span>
                                <span className="font-semibold">{payment.transaction_id}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-gray-600">{t('studentPaymentPage.amountLabel')}</span>
                                <span className="font-semibold">{payment.amount} {payment.currency}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-gray-600">{t('studentPaymentPage.paymentDateLabel')}</span>
                                <span className="font-semibold">
                                    {new Date(payment.paid_at).toLocaleDateString(language === 'ar' ? 'ar-EG' : 'en-US', {
                                        year: 'numeric',
                                        month: 'long',
                                        day: 'numeric',
                                        hour: '2-digit',
                            ...
                            <div className="flex justify-between">
                                <span className="text-gray-600">{t('studentPaymentPage.teacherLabel')}</span>
                                <span className="font-semibold">{payment.booking.teacher_name || t('common.notAvailable')}</span>
                            </div>
                        </div>
                    </div>
