...
export default function AchievementEdit({ achievement, types }) {
    ...
    const handleDelete = () => {
        if (confirm('هل أنت متأكد من حذف هذا الإنجاز؟ لا يمكن التراجع.')) {
            router.delete(route('innovation.achievements.destroy', achievement.id));
        }
    };

    return (
        <StudentPageShell title="تعديل الإنجاز" backHref={`/innovation/achievements/${achievement.id}`}>
            <div className="max-w-3xl mx-auto">
                <div className="mb-6">
                    <h1 className="text-2xl font-bold text-gray-900">✏️ تعديل الإنجاز</h1>
                    <p className="text-gray-500 mt-1">سيُعاد تحليل الإنجاز بالذكاء الاصطناعي بعد الحفظ</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {achievement.attachments?.length > 0 && (
                        <div className="bg-white rounded-2xl p-6 shadow-md border border-gray-100">
                            <label className="block text-sm font-bold text-gray-700 mb-3">📎 المرفقات الحالية</label>
                            <div className="space-y-2">
                                {achievement.attachments.map((att) => (
                                    <div key={att.id} className="flex items-center gap-2 p-3 bg-gray-50 rounded-xl">
                                        ...
                                        <span className="text-sm text-gray-700 truncate">{att.original_name || att.url || 'مرفق'}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
